...
This lab designs and configures a segmented office network for a company with four departments of different sizes. Rather than giving every department the same flat subnet (wasting hundreds of unused addresses on small teams), the network uses VLSM (Variable Length Subnet Masking) to size each subnet to its actual headcount, VLANs to logically isolate each department on the same physical switch, and router-on-a-stick (802.1Q trunking with router subinterfaces) to route between them using only one physical router port.
